...
df=pd.read_csv("btc_bars_2h.csv")
...
df1=pd.read_csv('btc_bars_2h.csv', usecols=["open","newvalue"])
...
dftest=pd.read_csv('btc_bars_2h_test.csv', usecols=["date","high","low","close","sma20","sma50","sma100","sma200","ema8","ema12","ema21","ema55","ema200","RSI"])
dftest['date'] = pd.to_datetime(dftest['date'], errors='coerce', format='%Y-%m-%d').dt.strftime("%Y%m%d")
valuestopredict=dftest.tail(9)
predictions = LR1.predict(valuestopredict)
